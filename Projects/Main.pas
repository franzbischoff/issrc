...
uses
  ...
  System.Generics.Collections, Quick.Config.Yaml;
...
type
  ...
  TSetupConfig = class(TAppConfigYaml)
  private
    fLang: string;
    fDir: string;
    fGroup: string;
    fNoIcons: Boolean;
    fSetupType: string;
    fComponents: TArray<string>;
    fTasks: TArray<string>;
  published
    [TCommentProperty('Sizes array is simple')]
    property Lang: string read fLang write fLang;
    property Dir: string read fDir write fDir;
    property Group: string read fGroup write fGroup;
    property NoIcons: Boolean read fNoIcons write fNoIcons;
    property SetupType: string read fSetupType write fSetupType;

    [TCommentProperty('List of Components')]
    property Components: TArray<string> read fComponents write fComponents;
    [TCommentProperty('List of Tasks')]
    property Tasks: TArray<string> read fTasks write fTasks;
  public
    destructor Destroy; override;
...
const
  ...
  EntryAnsiStrings: array [TEntryType] of Integer =
    (SetupLanguageEntryAnsiStrings, SetupCustomMessageEntryAnsiStrings,
    SetupPermissionEntryAnsiStrings, SetupTypeEntryAnsiStrings,
    SetupComponentEntryAnsiStrings, SetupTaskEntryAnsiStrings,
    SetupDirEntryAnsiStrings, SetupFileEntryAnsiStrings,
    SetupFileLocationEntryAnsiStrings, SetupIconEntryAnsiStrings,
    SetupIniEntryAnsiStrings, SetupRegistryEntryAnsiStrings,
    SetupDeleteEntryAnsiStrings, SetupDeleteEntryAnsiStrings,
...
var
  ...
  InitLoadYaml, InitSaveYaml: String;
  InitNoIcons, InitSilent, InitVerySilent, InitNoRestart, InitCloseApplications,
    InitNoCloseApplications, InitForceCloseApplications,
    InitNoForceCloseApplications, InitRestartApplications,
  ...
  { 'Constants' }
  SourceDir, TempInstallDir, WinDir, WinSystemDir, WinSysWow64Dir,
    WinSysNativeDir, SystemDrive, ProgramFiles32Dir, CommonFiles32Dir,
    ProgramFiles64Dir, CommonFiles64Dir, ProgramFilesUserDir,
    CommonFilesUserDir, SavedGamesUserDir, CmdFilename, SysUserInfoName,
  ...
  WizardUserInfoName, WizardUserInfoOrg, WizardUserInfoSerial, WizardDirValue,
    WizardGroupValue: String;
  WizardNoIcons, WizardPreparingYesRadio: Boolean;
  WizardSetupType: PSetupTypeEntry;
  WizardComponents, WizardDeselectedComponents, WizardTasks,
    WizardDeselectedTasks: TStringList;
  ...
  HasIcons, IsNT, IsWin64, Is64BitInstallMode, IsAdmin, IsPowerUserOrAdmin,
    IsAdminInstallMode, NeedPassword, NeedSerial, NeedsRestart, RestartSystem,
    IsUninstaller, AllowUninstallerShutdown,
    AcceptedQueryEndSessionInProgress: Boolean;
  ...
  ExpandedAppName, ExpandedAppVerName, ExpandedAppCopyright,
    ExpandedAppMutex: String;
...
function CodeRunnerOnDebugIntermediate(const Position: Longint;
  var ContinueStepOver: Boolean): Boolean;
procedure CodeRunnerOnDllImport(var DllName: String;
  var ForceDelayLoad: Boolean);
procedure CodeRunnerOnException(const Exception: AnsiString;
...
function ExpandConstEx(const S: String;
...
function GetCustomMessageValue(const AName: String; var AValue: String)
  : Boolean;
...
function GetUninstallRegSubkeyName(const UninstallRegKeyBaseName
  : String): String;
function GetPreviousData(const ExpandedAppId, ValueName, DefaultValueData
  : String): String;
...
function LoggedMsgBox(const Text, Caption: String; const Typ: TMsgBoxType;
  const Buttons: Cardinal; const Suppressible: Boolean;
  const Default: Integer): Integer;
function LoggedTaskDialogMsgBox(const Icon, Instruction, Text, Caption: String;
  const Typ: TMsgBoxType; const Buttons: Cardinal;
  const ButtonLabels: array of String; const ShieldButton: Integer;
  const Suppressible: Boolean; const Default: Integer): Integer;
...
function PreviousInstallCompleted(const WizardComponents,
  WizardTasks: TStringList): Boolean;
function CodeRegisterExtraCloseApplicationsResource(const DisableFsRedir
  : Boolean; const AFilename: String): Boolean;
procedure RegisterResourcesWithRestartManager(const WizardComponents,
  WizardTasks: TStringList);
...
function ShouldDisableFsRedirForFileEntry(const FileEntry
  : PSetupFileEntry): Boolean;
function ShouldDisableFsRedirForRunEntry(const RunEntry
  : PSetupRunEntry): Boolean;
...
function ShouldProcessFileEntry(const WizardComponents,
  WizardTasks: TStringList; const FileEntry: PSetupFileEntry;
  const IgnoreCheck: Boolean): Boolean;
function ShouldProcessIconEntry(const WizardComponents,
  WizardTasks: TStringList; const WizardNoIcons: Boolean;
  const IconEntry: PSetupIconEntry): Boolean;
...
{$IFDEF UNICODE}
    ...
    begin
      if Ord(Result[I]) > 126 then
      begin
        UseAnsiCRC32 := False;
        Break;
      end;
    end;
    if UseAnsiCRC32 then
    begin
      S := AnsiString(Result);
      FmtStr(Result, '%.48s~%.8x',
        [Result, GetCRC32(S[1], Length(S) * SizeOf(S[1]))]);
    end;
{$ELSE}
    FmtStr(Result, '%.48s~%.8x',
      [Result, GetCRC32(Result[1], Length(Result) * SizeOf(Result[1]))]);
{$ENDIF}
  end;
end;
...
function GetUninstallRegSubkeyName(const UninstallRegKeyBaseName
  : String): String;
begin
  Result := Format('%s\%s_is1', [NEWREGSTR_PATH_UNINSTALL,
    UninstallRegKeyBaseName]);
end;
...
begin
  ...
  begin
    if RegOpenKeyExView(InstallDefaultRegView, InstallModeRootKey,
      ...
      ), 0, KEY_QUERY_VALUE, H) = ERROR_SUCCESS then
    begin
      try
...
begin
  ...
  PrevLang := GetPreviousData(ExpandConst(SetupHeader.AppId),
    'Inno Setup: Language', '');
  ...
  begin
    ...
    begin
      if CompareText(PrevLang, PSetupLanguageEntry(Entries[seLanguage][I])
        .Name) = 0 then
      begin
        Result := I;
        Exit;
...
begin
  SHA1Init(Context);
  SHA1Update(Context, PAnsiChar('PasswordCheckHash')^,
    Length('PasswordCheckHash'));
  SHA1Update(Context, SetupHeader.PasswordSalt,
    SizeOf(SetupHeader.PasswordSalt));
  SHA1Update(Context, Pointer(Password)^, Length(Password) *
    SizeOf(Password[1]));
  Hash := SHA1Final(Context);
  Result := SHA1DigestsEqual(Hash, SetupHeader.PasswordHash);
end;

class function TDummyClass.ExpandCheckOrInstallConstant
  (Sender: TSimpleExpression; const Constant: String): String;
begin
  ...
  begin
    try
      ...
      try
        ...
        SimpleExpression.OnExpandConstant :=
          TDummyClass.ExpandCheckOrInstallConstant;
...
class function TDummyClass.EvalComponentOrTaskIdentifier
  (Sender: TSimpleExpression; const Name: String;
...
class function TDummyClass.EvalLanguageIdentifier(Sender: TSimpleExpression;
  const Name: String; const Parameters: array of const): Boolean;
begin
  Result := CompareText(PSetupLanguageEntry(Entries[seLanguage][ActiveLanguage])
    .Name, Name) = 0;
end;
...
class function TDummyClass.EvalCheckIdentifier(Sender: TSimpleExpression;
  const Name: String; const Parameters: array of const): Boolean;
begin
  Result := CodeRunner.RunBooleanFunction(AnsiString(Name), Parameters,
    True, False);
end;
...
begin
  if (Components <> '') or (Tasks <> '') or (Languages <> '') or (Check <> '')
  then
  begin
    if (Components <> '') and (WizardComponents <> nil) then
      ProcessComponent := EvalExpression(Components,
        TDummyClass.EvalComponentOrTaskIdentifier, Longint(WizardComponents))
    else
      ProcessComponent := True;

    if (Tasks <> '') and (WizardTasks <> nil) then
      ProcessTask := EvalExpression(Tasks,
        TDummyClass.EvalComponentOrTaskIdentifier, Longint(WizardTasks))
    else
      ProcessTask := True;

    if Languages <> '' then
      ProcessLanguage := EvalExpression(Languages,
        TDummyClass.EvalLanguageIdentifier, 0)
    else
...
function ShouldProcessFileEntry(const WizardComponents,
  ...
  const IgnoreCheck: Boolean): Boolean;
begin
  if foDontCopy in FileEntry.Options then
  begin
    Result := False;
    Exit;
  end;
  ...
  CheckOrInstallCurrentSourceFilename := FileEntry.SourceFilename;
  if IgnoreCheck then
    Result := ShouldProcessEntry(WizardComponents, WizardTasks,
      FileEntry.Components, FileEntry.Tasks, FileEntry.Languages, '')
  else
    Result := ShouldProcessEntry(WizardComponents, WizardTasks,
      FileEntry.Components, FileEntry.Tasks, FileEntry.Languages,
      FileEntry.Check);
...
begin
  if (InstallMode <> imNormal) and (roSkipIfSilent in RunEntry.Options) then
    Result := False
  else if (InstallMode = imNormal) and (roSkipIfNotSilent in RunEntry.Options)
  then
    Result := False
  else
    Result := ShouldProcessEntry(WizardComponents, WizardTasks,
      RunEntry.Components, RunEntry.Tasks, RunEntry.Languages, RunEntry.Check);
end;
...
begin
  if WizardNoIcons and (IconEntry.Tasks = '') and
    ...
    Result := ShouldProcessEntry(WizardComponents, WizardTasks,
      IconEntry.Components, IconEntry.Tasks, IconEntry.Languages,
      IconEntry.Check);
end;
...
begin
  ...
  if fo32Bit in FileEntry.Options then
    Result := False;
  if fo64Bit in FileEntry.Options then
  begin
    if not IsWin64 then
      InternalError
...
procedure LoadYaml(const Filename: String; var WantToSuppressMsgBoxes: Boolean);
var
  ConfigYaml: TSetupConfig;
  S: string;
begin
  ConfigYaml := TSetupConfig.Create(PChar(Filename), False);

  // if // check for file exists
  // begin
  // ConfigYaml.Load();
  // end;
  ...
  // ConfigYaml.Load();
  // // saved infs
  // InitLang := ConfigYaml.Lang;
  // InitDir := ConfigYaml.Dir;
  // InitProgramGroup := ConfigYaml.Group;
  // InitNoIcons := ConfigYaml.NoIcons;
  // InitSetupType := ConfigYaml.SetupType;
  // S := ConfigYaml.Components;
  // if S <> '$' then
  // begin
  ...
  // InitNoForceCloseApplications := GetIniBool(Section, 'NoForceCloseApplications', InitNoForceCloseApplications,
  // Filename);
...
begin
  ...
  InitProgramGroup := GetIniString(Section, 'Group', InitProgramGroup,
    Filename);
  ...
  InitVerySilent := GetIniBool(Section, 'VerySilent', InitVerySilent, Filename);
  InitNoRestart := GetIniBool(Section, 'NoRestart', InitNoRestart, Filename);
  InitCloseApplications := GetIniBool(Section, 'CloseApplications',
    InitCloseApplications, Filename);
  InitNoCloseApplications := GetIniBool(Section, 'NoCloseApplications',
    InitNoCloseApplications, Filename);
  InitForceCloseApplications := GetIniBool(Section, 'ForceCloseApplications',
    InitForceCloseApplications, Filename);
  InitNoForceCloseApplications := GetIniBool(Section,
    'NoForceCloseApplications', InitNoForceCloseApplications, Filename);
  InitRestartApplications := GetIniBool(Section, 'RestartApplications',
    InitRestartApplications, Filename);
  InitNoRestartApplications := GetIniBool(Section, 'NoRestartApplications',
    InitNoRestartApplications, Filename);
  InitNoCancel := GetIniBool(Section, 'NoCancel', InitNoCancel, Filename);
  InitPassword := GetIniString(Section, 'Password', InitPassword, Filename);
  InitRestartExitCode := GetIniInt(Section, 'RestartExitCode',
    InitRestartExitCode, 0, 0, Filename);
  WantToSuppressMsgBoxes := GetIniBool(Section, 'SuppressMsgBoxes',
    WantToSuppressMsgBoxes, Filename);
  InitSaveInf := GetIniString(Section, 'SaveInf', InitSaveInf, Filename);
end;

procedure SaveYaml(const Filename: String);
var
  ConfigYaml: TSetupConfig;
begin
  ConfigYaml := TSetupConfig.Create(PChar(Filename), False);
  ConfigYaml.Lang := PSetupLanguageEntry
    (Entries[seLanguage][ActiveLanguage]).Name;
  ConfigYaml.Dir := WizardDirValue;
  ConfigYaml.Group := WizardGroupValue;
  ConfigYaml.NoIcons := WizardNoIcons;
  if WizardSetupType <> nil then
  begin
    ConfigYaml.SetupType := WizardSetupType.Name;
    ConfigYaml.Components := WizardComponents.ToStringArray();
  end;
  ConfigYaml.Tasks := WizardTasks.ToStringArray();
  ConfigYaml.Save();
end;
...
begin
  ...
  begin
    ...
    SetIniString(Section, 'Components', StringsToCommaString(WizardComponents),
      Filename);
  end
  else
  begin
...
begin
  ...
  begin
    with PSetupCustomMessageEntry(Entries[seCustomMessage][I])^ do
    begin
      if (CompareText(Name, AName) = 0) and
        ((LangIndex = -1) or (LangIndex = ActiveLanguage)) then
      begin
        Result := True;
        AValue := Value;
...
{ Cnst must be the name of a single constant, without the braces.
  ...
  begin
    if Copy(Cnst, 1, 4) = 'auto' then
    begin
      StringChange(Cnst, 'auto', Actual[IsAdminInstallMode]);
      if (Cnst = 'userpf32') or (Cnst = 'userpf64') or (Cnst = 'usercf32') or
        (Cnst = 'usercf64') then
  ...
  begin
    InternalError
      (Format('Cannot evaluate "%s" constant during Uninstall', [C]));
  end;
  ...
  begin
    ...
    Result := '';
    if ConvertConstPercentStr(VarName) and ConvertConstPercentStr(Default) then
    begin
  ...
  const
    KeyNameConsts: array [0 .. 5] of TKeyNameConst = ((KeyName: 'HKA';
      KeyConst: HKEY_AUTO), (KeyName: 'HKCR'; KeyConst: HKEY_CLASSES_ROOT),
      (KeyName: 'HKCU'; KeyConst: HKEY_CURRENT_USER), (KeyName: 'HKLM';
      KeyConst: HKEY_LOCAL_MACHINE), (KeyName: 'HKU'; KeyConst: HKEY_USERS),
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (Z[L - 1] = '3') and (Z[L] = '2') then
          begin
            RegView := rv32Bit;
            SetLength(Z, L - 2);
          end
          else if (Z[L - 1] = '6') and (Z[L] = '4') then
          begin
            if not IsWin64 then
              InternalError
        ...
        for J := Low(KeyNameConsts) to High(KeyNameConsts) do
          if CompareText(KeyNameConsts[J].KeyName, Z) = 0 then
          begin
        ...
        begin
          ...
          begin
            ...
            if ConvertConstPercentStr(Subkey) and ConvertConstPercentStr(Value)
              and ConvertConstPercentStr(Default) then
            begin
              Result := ExpandConstEx(Default, CustomConsts);
              if RegOpenKeyExView(RegView, RootKey,
                PChar(ExpandConstEx(Subkey, CustomConsts)), 0, KEY_QUERY_VALUE,
                K) = ERROR_SUCCESS then
              begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if ConvertConstPercentStr(Filename) and
            ConvertConstPercentStr(Section) and ConvertConstPercentStr(Key) and
            ConvertConstPercentStr(Default) then
          begin
  ...
  begin
    ...
    SetLength(Z, I - 1);
    Param := Z;
    if ConvertConstPercentStr(Param) and ConvertConstPercentStr(Default) then
    begin
  ...
  { Expands an Pascal-script-value constant in the form:
    ...
    begin
      if (CodeRunner <> nil) then
        Result := CodeRunner.RunStringFunction(AnsiString(ScriptFunc),
  ...
  begin
    ...
    SetLength(Z, I - 1);
    ScriptFunc := Z;
    if ConvertConstPercentStr(ScriptFunc) and ConvertConstPercentStr(Default)
    then
    begin
      Default := ExpandConstEx(Default, CustomConsts);
  ...
  begin
    ...
    if ConvertConstPercentStr(C) then
    begin
  ...
  begin
    ...
    ArgCount := 0;
    while (I > 0) and (ArgCount <= High(ArgValues)) do
    begin
      Delete(C, 1, I);
      I := ConstPos(',', C);
      if I = 0 then
    ...
    if not GetCustomMessageValue(MsgName, Result) then
      InternalError(Format('Unknown custom message name "%s" in "cm" constant',
        [MsgName]));
...
const
  FolderConsts: array [Boolean, TShellFolderID] of String =
    ...
    'userfavorites', 'localappdata'), ('commondesktop', 'commonstartmenu',
    'commonprograms', 'commonstartup', 'usersendto', 'fonts', 'commonappdata',
    'commondocs', 'commontemplates',
    'commonfavorites' { not accepted anymore by the compiler } ,
    'localappdata'));
  NoUninstallConsts: array [0 .. 6] of String = ('src', 'srcexe',
    'userinfoname', 'userinfoorg', 'userinfoserial', 'hwnd', 'wizardhwnd');
var
...
begin
  ...
  if Cnst = '\' then
    Result := '\'
  else if Cnst = 'app' then
  begin
    if IsUninstaller then
    begin
      if UninstallExpandedApp = '' then
        InternalError('An attempt was made to expand the "' + OriginalCnst +
          '" constant but Setup didn''t create the "app" dir');
  ...
  else if Cnst = 'win' then
    Result := WinDir
  else if Cnst = 'sys' then
    Result := WinSystemDir
  else if Cnst = 'syswow64' then
  begin
    ...
    begin
      if IsWin64 then { sanity check }
        InternalError('Cannot expand "' + OriginalCnst +
          '" constant because there is no SysWOW64 directory');
      Result := WinSystemDir;
    end;
  end
  else if Cnst = 'sysnative' then
  begin
  ...
  else if Cnst = 'src' then
    Result := SourceDir
  else if Cnst = 'srcexe' then
    Result := SetupLdrOriginalFilename
  else if Cnst = 'tmp' then
    Result := TempInstallDir
  else if Cnst = 'sd' then
    Result := SystemDrive
  else if Cnst = 'userpf' then
  begin
    if ProgramFilesUserDir <> '' then
      Result := ProgramFilesUserDir
    else
      Result := ExpandConst('{localappdata}\Programs');
    { supply default, same as Window 7 and newer }
  end
  else if Cnst = 'commonpf' then
  begin
  ...
  else if Cnst = 'usercf' then
  begin
    if CommonFilesUserDir <> '' then
      Result := CommonFilesUserDir
    else
      Result := ExpandConst('{localappdata}\Programs\Common');
    { supply default, same as Window 7 and newer }
  end
  else if Cnst = 'commoncf' then
  begin
    if Is64BitInstallMode then
  ...
  else if Cnst = 'commonpf32' then
    Result := ProgramFiles32Dir
  else if Cnst = 'commoncf32' then
    Result := CommonFiles32Dir
  else if Cnst = 'commonpf64' then
  begin
    if IsWin64 then
      Result := ProgramFiles64Dir
    else
  ...
  else if Cnst = 'usersavedgames' then
    Result := SavedGamesUserDir
  else if Cnst = 'dao' then
    Result := ExpandConst('{cf}\Microsoft Shared\DAO')
  else if Cnst = 'cmd' then
    Result := CmdFilename
  else if Cnst = 'computername' then
    Result := GetComputerNameString
  else if Cnst = 'username' then
    Result := GetUserNameString
  else if Cnst = 'groupname' then
  begin
    if IsUninstaller then
    begin
      if UninstallExpandedGroupName = '' then
        InternalError('Cannot expand "' + OriginalCnst +
          '" constant because it was not available at install time');
  ...
  else if Cnst = 'sysuserinfoname' then
    Result := SysUserInfoName
  else if Cnst = 'sysuserinfoorg' then
    Result := SysUserInfoOrg
  else if Cnst = 'userinfoname' then
    Result := WizardUserInfoName
  else if Cnst = 'userinfoorg' then
    Result := WizardUserInfoOrg
  else if Cnst = 'userinfoserial' then
    Result := WizardUserInfoSerial
  else if Cnst = 'uninstallexe' then
    Result := UninstallExeFilename
  else if Cnst = 'group' then
  begin
    if IsUninstaller then
    begin
    ...
    begin
      ...
      ShellFolder := GetShellFolder
        (not(shAlwaysUsePersonalGroup in SetupHeader.Options) and
        IsAdminInstallMode, sfPrograms, False);
      if ShellFolder = '' then
        InternalError('Failed to expand "' + OriginalCnst + '" constant');
      Result := AddBackslash(ShellFolder) + WizardGroupValue;
    end;
  end
  else if Cnst = 'language' then
  begin
    if IsUninstaller then
  ...
  else if Cnst = 'log' then
    Result := GetLogFileName
  else if Cnst = 'dotnet11' then
    Result := GetDotNetVersionRoot(rv32Bit, dt11)
  else if Cnst = 'dotnet20' then
    Result := GetDotNetVersionRoot(InstallDefaultRegView, dt20)
  else if Cnst = 'dotnet2032' then
    Result := GetDotNetVersionRoot(rv32Bit, dt20)
  else if Cnst = 'dotnet2064' then
  begin
    if IsWin64 then
  ...
  else if Cnst = 'dotnet40' then
    Result := GetDotNetVersionRoot(InstallDefaultRegView, dt40)
  else if Cnst = 'dotnet4032' then
    Result := GetDotNetVersionRoot(rv32Bit, dt40)
  else if Cnst = 'dotnet4064' then
  begin
    if IsWin64 then
  ...
  else if (Cnst <> '') and (Cnst[1] = '%') then
    Result := ExpandEnvConst(Cnst)
  else if StrLComp(PChar(Cnst), 'reg:', 4) = 0 then
    Result := ExpandRegConst(Cnst)
  else if StrLComp(PChar(Cnst), 'ini:', 4) = 0 then
    Result := ExpandIniConst(Cnst)
  else if StrLComp(PChar(Cnst), 'param:', 6) = 0 then
    Result := ExpandParamConst(Cnst)
  else if StrLComp(PChar(Cnst), 'code:', 5) = 0 then
    Result := ExpandCodeConst(Cnst)
  else if StrLComp(PChar(Cnst), 'drive:', 6) = 0 then
    Result := ExpandDriveConst(Cnst)
  else if StrLComp(PChar(Cnst), 'cm:', 3) = 0 then
    Result := ExpandCustomMessageConst(Cnst)
  else
  begin
    { Shell folder constants }
    for Common := False to True do
      for ShellFolderID := Low(ShellFolderID) to High(ShellFolderID) do
        if Cnst = FolderConsts[Common, ShellFolderID] then
        begin
          ShellFolder := GetShellFolder(Common, ShellFolderID, False);
          if ShellFolder = '' then
            InternalError(Format('Failed to expand shell folder constant "%s"',
              [OriginalCnst]));
          Result := ShellFolder;
          Exit;
        end;
    ...
    begin
      I := 0;
      while I < High(CustomConsts) do
      begin
        if Cnst = CustomConsts[I] then
        begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if DoExpandIndividualConst then
        begin
...
procedure InitMainNonSHFolderConsts;
  ...
  begin
    if RegOpenKeyExView(RegView, HKEY_LOCAL_MACHINE, NEWREGSTR_PATH_SETUP, 0,
      KEY_QUERY_VALUE, H) = ERROR_SUCCESS then
    begin
  ...
  begin
    ...
    if IsWin64 then
      ...
      RegView := rvDefault;
    if RegOpenKeyExView(RegView, HKEY_LOCAL_MACHINE, Paths[IsNT], 0,
      KEY_QUERY_VALUE, K) = ERROR_SUCCESS then
    begin
...
const
  FOLDERID_UserProgramFiles: TGUID = (D1: $5CD7AEE2; D2: $2219; D3: $4A67;
    D4: ($B8, $5D, $6C, $9C, $E1, $56, $60, $CB));
  FOLDERID_UserProgramFilesCommon: TGUID = (D1: $BCBD3057; D2: $CA5C; D3: $4622;
    D4: ($B4, $2D, $BC, $56, $DB, $0A, $E5, $16));
  FOLDERID_SavedGames: TGUID = (D1: $4C5C32FF; D2: $BB9D; D3: $43B0;
    D4: ($B5, $B4, $2D, $72, $E5, $4E, $AA, $A4));
...
begin
  ...
  if CommonFiles32Dir = '' then
    CommonFiles32Dir := AddBackslash(ProgramFiles32Dir) + 'Common Files';
  { don't localize }
  ...
  if Assigned(SHGetKnownFolderPathFunc) and (WindowsVersion shr 16 >= $0600)
  then
  begin
    if SHGetKnownFolderPathFunc(FOLDERID_UserProgramFiles { Windows 7+ } ,
      KF_FLAG_CREATE, 0, Path) = S_OK then
    begin
      try
    ...
    if SHGetKnownFolderPathFunc(FOLDERID_UserProgramFilesCommon { Windows 7+ } ,
      KF_FLAG_CREATE, 0, Path) = S_OK then
    begin
      try
    ...
    if SHGetKnownFolderPathFunc(FOLDERID_SavedGames { Vista+ } , KF_FLAG_CREATE,
      0, Path) = S_OK then
    begin
      try
...
begin
  ...
  if not CreateDirectory(PChar(Subdir), nil) then
  begin
    ErrorCode := GetLastError;
    raise Exception.Create(FmtSetupMessage(msgLastErrorMessage,
...
Retry:
  ...
  if not Result and (GetLastError <> ERROR_FILE_NOT_FOUND) and
  ...
  begin
    ...
    if Cardinal(Elapsed) < Cardinal(2000) then
    begin
      Sleep(50);
      goto Retry;
    end;
...
begin
  ...
  @SHGetFolderPathFunc := GetProcAddress(SHFolderDLLHandle,
...
begin
  ...
  if (WindowsVersion shr 16 >= $0600) and (Folder and CSIDL_FLAG_CREATE <> 0)
  then
...
const
  ...
  FolderIDs: array [Boolean, TShellFolderID] of Integer = (
    ...
    CSIDL_SENDTO, CSIDL_FONTS, CSIDL_APPDATA, CSIDL_PERSONAL, CSIDL_TEMPLATES,
    CSIDL_FAVORITES, CSIDL_LOCAL_APPDATA),
    { Common }
    (CSIDL_COMMON_DESKTOPDIRECTORY, CSIDL_COMMON_STARTMENU,
    CSIDL_COMMON_PROGRAMS, CSIDL_COMMON_STARTUP, CSIDL_SENDTO, CSIDL_FONTS,
    CSIDL_COMMON_APPDATA, CSIDL_COMMON_DOCUMENTS, CSIDL_COMMON_TEMPLATES,
    CSIDL_COMMON_FAVORITES, CSIDL_LOCAL_APPDATA));
var
  ShellFolder: String;
begin
  if not ShellFoldersRead[Common, ID] then
  begin
      ...
      begin
        ...
        begin
          ...
          if (Ver2 > OnlyBelowVer) or
            ((Ver2 = OnlyBelowVer) and (OnlyBelowVersion.NTServicePack = 0)) or
...
function GetSizeOfComponent(const ComponentName: String;
  const ExtraDiskSpaceRequired: Integer64): Integer64;
var
...
begin
  ...
  try
    ...
    begin
      ...
      with FileEntry^ do
      begin
        if (Components <> '') and ((Tasks = '') and (Check = '')) then
        begin { don't count tasks or scripted entries }
          if ShouldProcessFileEntry(ComponentNameAsList, nil, FileEntry, True)
          then
          begin
...
function GetSizeOfType(const TypeName: String; const IsCustom: Boolean)
  : Integer64;
var
...
type
  TEnumFilesProc = function(const DisableFsRedir: Boolean;
    const Filename: String; const Param: Pointer): Boolean;

function DummyDeleteDirProc(const DisableFsRedir: Boolean;
  const Filename: String; const Param: Pointer): Boolean;
begin
...
function EnumFiles(const EnumFilesProc: TEnumFilesProc;
  const WizardComponents, WizardTasks: TStringList;
  ...
  begin
    ...
    if foRecurseSubDirsExternal in CurFile^.Options then
    begin
      H := FindFirstFileRedir(DisableFsRedir, SearchBaseDir + SearchSubDir +
        '*', FindData);
      if H <> INVALID_HANDLE_VALUE then
      begin
        try
...
begin
  ...
  begin
    ...
    if (CurFile^.FileType = ftUserFile) and
      ShouldProcessFileEntry(WizardComponents, WizardTasks, CurFile, False) then
    begin
      DisableFsRedir := ShouldDisableFsRedirForFileEntry(CurFile);
      if CurFile^.LocationEntry <> -1 then
      begin
        { Non-external file }
        if not EnumFilesProc(DisableFsRedir, ExpandConst(CurFile^.DestName),
          Param) then
        begin
          Result := False;
          Exit;
      ...
      begin
        ...
        if not RecurseExternalFiles(DisableFsRedir,
          PathExtractPath(SourceWildcard), '', PathExtractName(SourceWildcard),
          IsWildcard(SourceWildcard), CurFile) then
        begin
          Result := False;
          Exit;
        end;
  ...
  for I := 0 to Entries[seInstallDelete].Count - 1 do
    with PSetupDeleteEntry(Entries[seInstallDelete][I])^ do
      if ShouldProcessEntry(WizardComponents, WizardTasks, Components, Tasks,
        Languages, Check) then
      begin
        case DeleteType of
          dfFiles, dfFilesAndOrSubdirs:
            if not DelTree(InstallDefaultDisableFsRedir, ExpandConst(Name),
              False, True, DeleteType = dfFilesAndOrSubdirs, True,
              DummyDeleteDirProc, EnumFilesProc, nil) then
            begin
              Result := False;
              Exit;
            end;
          dfDirIfEmpty:
            if not DelTree(InstallDefaultDisableFsRedir, ExpandConst(Name),
              True, False, False, True, DummyDeleteDirProc, EnumFilesProc, nil)
            then
            begin
              Result := False;
              Exit;
...
begin
  ...
  for J := 0 to CheckForFileSL.Count - 1 do
  begin
    if CheckForFileSL[J] = Filename then
    begin
      LogFmt('Found pending rename or delete that matches one of our files: %s',
        [Filename]);
...
type
  TArrayOfPWideChar = array [0 .. (Maxint div SizeOf(PWideChar)) - 1]
    of PWideChar;
...
begin
  ...
  begin
    ...
    begin
      ...
      for I := 0 to CloseApplicationsFilterList.Count - 1 do
      begin
        if WildcardMatch(PChar(Text), PChar(CloseApplicationsFilterList[I]))
        then
        begin
          Match := True;
          Break;
  ...
  if ((Filename <> '') and
    (RegisterFileFilenamesCount = RegisterFileFilenamesMax)) or
    ((Filename = '') and (RegisterFileFilenamesCount > 0)) then
  begin
    if RmRegisterResources(RmSessionHandle, RegisterFileFilenamesCount,
      RegisterFileFilenames, 0, nil, 0, nil) = ERROR_SUCCESS then
    begin
  ...
  if RmSessionStarted and (Filename <> '') then
  begin
    ...
    Len := Length(Filename);
    GetMem(RegisterFileFilenames[RegisterFileFilenamesCount],
      (Len + 1) * SizeOf(RegisterFileFilenames[RegisterFileFilenamesCount][0]));
{$IFNDEF UNICODE}
    RegisterFileFilenames[RegisterFileFilenamesCount]
      [MultiByteToWideChar(CP_ACP, 0, PChar(Filename), Len,
      RegisterFileFilenames[RegisterFileFilenamesCount], Len)] := #0;
{$ELSE}
...
{$ENDIF}
  ...
  Result := RmSessionStarted;
  { Break the enum if there was an error, else continue. }
end;
...
begin
  ...
  begin
    InternalError
      ('Cannot call "RegisterExtraCloseApplicationsResource" function at this time');
    Result := False;
  end;
end;
...
begin
  ...
  GetMem(RegisterFileFilenames, RegisterFileFilenamesMax *
    SizeOf(RegisterFileFilenames[0]));
  try
    { Register our files. }
    ...
    begin
      ...
      try
        try
          CodeRunner.RunProcedures('RegisterExtraCloseApplicationsResources',
            [''], False);
        except
...
begin
  ...
  case EntryType of
    ...
    seRegistry:
      Kind := deRegistry;
    seInstallDelete:
      Kind := deInstallDelete;
    seUninstallDelete:
      Kind := deUninstallDelete;
    seRun:
      Kind := deRun;
    seUninstallRun:
      Kind := deUninstallRun;
...
begin
  ...
  begin
    ...
    else if Pos('uninstall:', DllName) = 1 then
    begin
...
{$ENDIF}
  ...
  SetMessageBoxCaption(mbInformation,
    PChar(SetupMessages[msgInformationTitle]));
  ...
  for J := 0 to Entries[seType].Count - 1 do
  begin
    with PSetupTypeEntry(Entries[seType][J])^ do
    begin
      case Typ of
        ttDefaultFull:
          Description := SetupMessages[msgFullInstallation];
        ttDefaultCompact:
          Description := SetupMessages[msgCompactInstallation];
        ttDefaultCustom:
          Description := SetupMessages[msgCustomInstallation];
      end;
    end;
  end;
...
function GetLanguageEntryProc(Index: Integer;
  var Entry: PSetupLanguageEntry): Boolean;
begin
  Result := False;
  if Index < Entries[seLanguage].Count then
  begin
...
begin
  ...
  case DetermineDefaultLanguage(GetLanguageEntryProc,
    ...
    ddNoMatch:
      ShowLanguageDialog := (SetupHeader.ShowLanguageDialog <> slNo);
    ddMatch:
...
begin
  { The taskbar button is hidden by setting the WS_EX_TOOLWINDOW style on the
    ...
    is shown. }
{$IFDEF IS_D12}
  TaskbarButtonHidden := not AVisible;
  { see WM_STYLECHANGING hook in Setup.dpr }
{$ENDIF}
  if (GetWindowLong(Application.Handle, GWL_EXSTYLE) and WS_EX_TOOLWINDOW = 0)
    <> AVisible then
  begin
    SetWindowPos(Application.Handle, 0, 0, 0, 0, 0, SWP_NOSIZE or SWP_NOMOVE or
...
var
  SP: String;
begin
  if NTServicePackLevel <> 0 then
  begin
    SP := ' SP' + IntToStr(Hi(NTServicePackLevel));
    if Lo(NTServicePackLevel) <> 0 then
      SP := SP + '.' + IntToStr(Lo(NTServicePackLevel));
  end;
  LogFmt('Windows version: %u.%u.%u%s  (NT platform: %s)',
    [WindowsVersion shr 24, (WindowsVersion shr 16) and $FF, WindowsVersion and
    $FFFF, SP, SYesNo[IsNT]]);
  LogFmt('64-bit Windows: %s', [SYesNo[IsWin64]]);
  LogFmt('Processor architecture: %s',
...
begin
  case Buttons and MB_TYPEMASK of
    ...
    MB_CANCELTRYCONTINUE:
      Result := 'Cancel/Try Again/Continue';
  else
...
begin
  Log(Format('Message box (%s):' + SNewLine, [GetButtonsText(Buttons)]) + Text);
end;

function LoggedAppMessageBox(const Text, Caption: PChar; const Flags: Longint;
  const Suppressible: Boolean; const Default: Integer): Integer;
begin
  if InitSuppressMsgBoxes and Suppressible then
  begin
...
begin
  ...
  begin
    LogMessageBox(PChar(Text), Buttons);
    Result := TaskDialogMsgBox(Icon, Instruction, Text, Caption, Typ, Buttons,
      ButtonLabels, ShieldButton);
    if Result <> 0 then
      LogFmt('User chose %s.', [GetMessageBoxResultText(Result)])
    else
...
begin
  ...
  begin
    ...
    LoggedMsgBox(SetupMessages[msgErrorRestartingComputer], '', mbError, MB_OK,
      True, IDOK);
  end;
end;
...
var
  ...
  Server: TSpawnServer;
  ParamNotifyWnd: HWND;
  RespawnResults: record ExitCode: DWORD;
...
begin
  ...
  if not SetupNotifyWndPresent then
  begin
    ...
    try
      ...
      if RespawnResults.NotifyRestartRequested then
      begin
...
var
  ...
  function ArchitecturesToStr(const Architectures
    : TSetupProcessorArchitectures): String;
  ...
  begin
    LoggedMsgBox(FmtSetupMessage(Msg, [Arg1]), '', mbCriticalError, MB_OK,
      True, IDOK);
    Abort;
  end;
  ...
  begin
    LoggedMsgBox(FmtSetupMessage(msgWindowsServicePackRequired,
      ['Windows', IntToStr(Hi(ServicePack))]), '', mbCriticalError, MB_OK,
      True, IDOK);
    Abort;
  end;
...
var
  ...
  begin
    ...
    begin
      ...
      if (MinVersionOfs = -1) or
        ...
        ^), TSetupVersionData((@PByteArray(P)[OnlyBelowVersionOfs])^))
        = irInstall) then
      begin
        Entries[EntryType].Add(P);
        if Debugging then
  ...
  begin
    ...
    if NeedPassword and (InitPassword <> '') then
    begin
      PasswordOk := False;
      S := InitPassword;
      ...
      if not PasswordOk and (CodeRunner <> nil) then
        PasswordOk := CodeRunner.RunBooleanFunctions('CheckPassword', [S],
          bcTrue, False, PasswordOk);
  ...
  begin
    ...
    begin
      if InstallMode = imVerySilent then
      begin
...
{ Also see Install.pas }
  function ExistingInstallationAt(const RootKey: HKEY;
    const SubkeyName: String): Boolean;
  var
    K: HKEY;
  begin
    if RegOpenKeyExView(InstallDefaultRegView, RootKey, PChar(SubkeyName), 0,
      KEY_QUERY_VALUE, K) = ERROR_SUCCESS then
    begin
      Result := True;
      RegCloseKey(K);
  ...
  var
    ExistingAtAdminInstallMode, ExistingAtNonAdminInstallMode,
      DesireAnInstallMode, DesireAdminInstallMode: Boolean;
    SubkeyName, AppName: String;
  begin
    if HasInitPrivilegesRequired and
      (proCommandLine in SetupHeader.PrivilegesRequiredOverridesAllowed) then
    begin
    ...
    else if proDialog in SetupHeader.PrivilegesRequiredOverridesAllowed then
    begin
      if shUsePreviousPrivileges in SetupHeader.Options then
      begin
        ...
        SubkeyName := GetUninstallRegSubkeyName
          (GetUninstallRegKeyBaseName(ExpandConst(SetupHeader.AppId)));
        ExistingAtAdminInstallMode := ExistingInstallationAt(HKEY_LOCAL_MACHINE,
          SubkeyName);
        ExistingAtNonAdminInstallMode :=
          ExistingInstallationAt(HKEY_CURRENT_USER, SubkeyName);
      end
      else
      begin
      ...
      else if not ExistingAtAdminInstallMode and ExistingAtNonAdminInstallMode
      then
        DesireAdminInstallMode := False
      else if not InitSuppressMsgBoxes then
      begin
        ...
        if shAppNameHasConsts in SetupHeader.Options then
          AppName := PathChangeExt
            (PathExtractName(SetupLdrOriginalFilename), '')
        else
          AppName := SetupHeader.AppName;
        if SetupHeader.PrivilegesRequired = prLowest then
        begin
          case TaskDialogMsgBox('MAINICON',
            SetupMessages[msgPrivilegesRequiredOverrideInstruction],
            FmtSetupMessage(msgPrivilegesRequiredOverrideText2, [AppName]),
            SetupMessages[msgPrivilegesRequiredOverrideTitle], mbInformation,
            MB_YESNOCANCEL,
            [SetupMessages[msgPrivilegesRequiredOverrideCurrentUserRecommended],
            SetupMessages[msgPrivilegesRequiredOverrideAllUsers]], IDNO) of
            IDYES:
              DesireAdminInstallMode := False;
            IDNO:
              DesireAdminInstallMode := True;
        ...
        begin
          case TaskDialogMsgBox('MAINICON',
            ...
            SetupMessages[msgPrivilegesRequiredOverrideTitle], mbInformation,
            MB_YESNOCANCEL,
            [SetupMessages[msgPrivilegesRequiredOverrideAllUsersRecommended],
            SetupMessages[msgPrivilegesRequiredOverrideCurrentUser]], IDYES) of
            IDYES:
              DesireAdminInstallMode := True;
            IDNO:
              DesireAdminInstallMode := False;
        ...
        DesireAnInstallMode := False;
      { No previous found and msgboxes are suppressed, just keep things as they are. }

      if DesireAnInstallMode then
      begin
        if DesireAdminInstallMode then
        begin
...
begin
  ...
  DebugWndValue := 0;
  for I := StartParam to NewParamCount do
  begin
    SplitNewParamStr(I, ParamName, ParamValue);
    if CompareText(ParamName, '/Log') = 0 then
    begin
      EnableLogging := True;
      LogFilename := '';
    end
    else if CompareText(ParamName, '/Log=') = 0 then
    begin
      EnableLogging := True;
    ...
    else if CompareText(ParamName, '/Components=') = 0 then
    begin
      InitComponentsSpecified := True;
      SetStringsFromCommaString(InitComponents,
        SlashesToBackslashes(ParamValue));
    end
    else if CompareText(ParamName, '/Tasks=') = 0 then
    begin
      InitDeselectAllTasks := True;
      SetStringsFromCommaString(InitTasks, SlashesToBackslashes(ParamValue));
    end
    else if CompareText(ParamName, '/MergeTasks=') = 0 then
    begin
      InitDeselectAllTasks := False;
      SetStringsFromCommaString(InitTasks, SlashesToBackslashes(ParamValue));
    end
    else if CompareText(ParamName, '/LoadInf=') = 0 then
      InitLoadInf := PathExpand(ParamValue)
    else if CompareText(ParamName, '/SaveInf=') = 0 then
      InitSaveInf := PathExpand(ParamValue)
    else if CompareText(ParamName, '/LoadYaml=') = 0 then
      InitLoadYaml := PathExpand(ParamValue)
    else if CompareText(ParamName, '/SaveYaml=') = 0 then
      InitSaveYaml := PathExpand(ParamValue)
  ...
  if InitLoadYaml <> '' then
    LoadYaml(InitLoadYaml, WantToSuppressMsgBoxes);
  ...
  try
    ...
    try
      ...
      try
        ...
        ReadEntriesWithoutVersion(seCustomMessage,
          SetupHeader.NumCustomMessageEntries,
          SizeOf(TSetupCustomMessageEntry));
        { Permission entries }
        ReadEntriesWithoutVersion(sePermission,
        ...
        if ProcessorArchitecture in SetupHeader.ArchitecturesInstallIn64BitMode
        then
        begin
          if not IsWin64 then
          begin
        ...
        if not IsRespawnedProcess and NeedToRespawnSelfElevated
          (not(SetupHeader.PrivilegesRequired in [prNone, prLowest]),
        ...
        ShutdownBlockReasonCreate(Application.Handle,
          SetupMessages[msgWizardInstalling]);
        ...
        { Initialize logging }
        if EnableLogging or (shSetupLogging in SetupHeader.Options) then
        begin
          try
            if LogFilename = '' then
              StartLogging('Setup')
        ...
        { Component entries }
        ReadEntries(seComponent, SetupHeader.NumComponentEntries,
          SizeOf(TSetupComponentEntry), -1, -1);
        { Task entries }
        ReadEntries(seTask, SetupHeader.NumTaskEntries,
          SizeOf(TSetupTaskEntry), -1, -1);
        { Dir entries }
        ...
        { Registry entries }
        ReadEntries(seRegistry, SetupHeader.NumRegistryEntries,
          SizeOf(TSetupRegistryEntry),
          Integer(@PSetupRegistryEntry(nil).MinVersion),
          Integer(@PSetupRegistryEntry(nil).OnlyBelowVersion));
        { InstallDelete entries }
        ReadEntries(seInstallDelete, SetupHeader.NumInstallDeleteEntries,
          SizeOf(TSetupDeleteEntry),
          Integer(@PSetupDeleteEntry(nil).MinVersion),
          Integer(@PSetupDeleteEntry(nil).OnlyBelowVersion));
        { UninstallDelete entries }
        ReadEntries(seUninstallDelete, SetupHeader.NumUninstallDeleteEntries,
          SizeOf(TSetupDeleteEntry),
          Integer(@PSetupDeleteEntry(nil).MinVersion),
          Integer(@PSetupDeleteEntry(nil).OnlyBelowVersion));
        { Run entries }
        ...
        ReadEntries(seUninstallRun, SetupHeader.NumUninstallRunEntries,
          SizeOf(TSetupRunEntry), Integer(@PSetupRunEntry(nil).MinVersion),
          Integer(@PSetupRunEntry(nil).OnlyBelowVersion));
      ...
      try
        ...
        ReadEntriesWithoutVersion(seFileLocation,
          SetupHeader.NumFileLocationEntries, SizeOf(TSetupFileLocationEntry));
      finally
        Reader.Free;
      end;
    except
  ...
  InitializeAdminInstallMode(IsAdmin and (SetupHeader.PrivilegesRequired <>
    prLowest));
  ...
  { Show "Select Language" dialog if necessary - requires "64-bit mode" to be
    ...
    view }
  if Entries[seLanguage].Count > 1 then
  begin
    if ShowLanguageDialog and not InitSilent and not InitVerySilent then
    begin
      if not AskForLanguage then
        Abort;
    end
    else if not MatchedLangParameter and
      (shUsePreviousLanguage in SetupHeader.Options) then
    begin
  ...
  if (SetupHeader.ArchitecturesAllowed <> []) and
    ...
    AbortInitFmt1(msgOnlyOnTheseArchitectures,
      ArchitecturesToStr(SetupHeader.ArchitecturesAllowed));

  { Check Windows version }
  case InstallOnThisVersion(SetupHeader.MinVersion,
    SetupHeader.OnlyBelowVersion) of
    irInstall:
      ;
    irServicePackTooLow:
  ...
  case SetupHeader.PrivilegesRequired of
    prPowerUser:
      if not IsPowerUserOrAdmin then
        AbortInit(msgPowerUserPrivilegesRequired);
    prAdmin:
      if not IsAdmin then
  ...
  if InitCloseApplications or ((shCloseApplications in SetupHeader.Options) and
    not InitNoCloseApplications) then
  begin
    ...
    { Note from Old New Thing: "The RmStartSession function doesn't properly
      ...
      it's global. }
    if UseRestartManager and (RmStartSession(@RmSessionHandle, 0, RmSessionKey)
      = ERROR_SUCCESS) then
    begin
      RmSessionStarted := True;
      SetStringsFromCommaString(CloseApplicationsFilterList,
        SetupHeader.CloseApplicationsFilter);
    end;
  end;
  ...
  begin
    ...
    try
      ...
      CodeRunner.LoadScript(SetupHeader.CompiledCodeText,
    ...
    try
      Res := CodeRunner.RunBooleanFunctions('InitializeSetup', [''], bcFalse,
        False, True);
    except
  ...
  while CheckForMutexes(ExpandedAppMutex) do
    if LoggedMsgBox(FmtSetupMessage1(msgSetupAppRunningError, ExpandedAppName),
      SetupMessages[msgSetupAppTitle], mbError, MB_OKCANCEL, True, IDCANCEL) <> IDOK
    then
      Abort;

  { Check if Setup is running and if not create mutexes }
  while CheckForMutexes(ExpandedSetupMutex) do
    if LoggedMsgBox(FmtSetupMessage1(msgSetupAppRunningError,
      SetupMessages[msgSetupAppTitle]), SetupMessages[msgSetupAppTitle],
  ...
  begin
    if not ShouldProcessEntry(nil, nil, '', '',
      PSetupTypeEntry(Entries[seType][I]).Languages,
      PSetupTypeEntry(Entries[seType][I]).Check) then
    begin
      SEFreeRec(Entries[seType][I], EntryStrings[seType],
        EntryAnsiStrings[seType]);
  ...
  begin
    ...
    if (ComponentEntry.Level <= NextAllowedLevel) and
      (InstallOnThisVersion(ComponentEntry.MinVersion,
      ComponentEntry.OnlyBelowVersion) = irInstall) and
      ShouldProcessEntry(nil, nil, '', '', ComponentEntry.Languages,
      ComponentEntry.Check) then
    begin
    ...
    begin
      ...
      if Assigned(LastShownComponentEntry) and
        ...
        (CompareText(ComponentEntry.Name, LastShownComponentEntry.Name) = 0)
      then
      begin
      ...
      SEFreeRec(ComponentEntry, EntryStrings[seComponent],
        EntryAnsiStrings[seComponent]);
    end;
  end;
  ...
  for I := 0 to Entries[seType].Count - 1 do
  begin
    if toIsCustom in PSetupTypeEntry(Entries[seType][I]).Options then
    begin
      HasCustomType := True;
      Break;
    end;
  ...
  begin
    ...
    begin
      if LocationEntry <> -1 then
      begin { not an "external" file }
        if Components = ''
        then { no types or a file that doesn't belong to any component }
          if (Tasks = '') and (Check = '')
          then { don't count tasks and scripted entries }
            Inc6464(MinimumSpace,
              PSetupFileLocationEntry(Entries[seFileLocation][LocationEntry])
              ^.OriginalSize)
      end
      else
      begin
        ...
        begin
          try
            ...
            ExternalSize := RecurseExternalGetSizeOfFiles
              (ShouldDisableFsRedirForFileEntry(PSetupFileEntry(Entries[seFile]
              [I])), PathExtractPath(SourceWildcard), '',
        ...
        then { no types or a file that doesn't belong to any component }
          if (Tasks = '') and (Check = '')
          then { don't count tasks or scripted entries }
  ...
  begin
    for I := 0 to Entries[seType].Count - 1 do
    begin
      with PSetupTypeEntry(Entries[seType][I])^ do
      begin
...
begin
  ...
  if Assigned(CodeRunner) then
  begin
    if AllowCustomSetupExitCode then
    begin
      try
        SetupExitCode := CodeRunner.RunIntegerFunctions
          ('GetCustomSetupExitCode', [''], bcNonZero, False, SetupExitCode);
      except
  ...
  { An attempt to restart while debugging is most likely an accident;
    don't allow it }
  if RestartSystem and Debugging then
  begin
...
begin
  ...
  for I := 0 to 2 do
    TColorBytes(Result)[I] :=
      Integer(TColorBytes(Color1)[I] +
      ((TColorBytes(Color2)[I] - TColorBytes(Color1)[I]) * Blend) div 255);
end;

function ExitSetupMsgBox: Boolean;
begin
  Result := LoggedMsgBox(SetupMessages[msgExitSetupMessage],
    SetupMessages[msgExitSetupTitle], mbConfirmation, MB_YESNO or MB_DEFBUTTON2,
    False, 0) = IDYES;
end;
...
procedure TSetupConfig.Init;
begin
  inherited;
//  fComponents := TStrings.Create();
//  fTasks := TStrings.Create();
  DefaultValues();
end;

procedure TSetupConfig.DefaultValues;
begin
  inherited;
  fSetupType := '(Default)';
end;

destructor TSetupConfig.Destroy;
begin
...
begin
  ...
  begin
    ...
    if not(shWindowShowCaption in SetupHeader.Options) then
      BorderStyle := bsNone
    else if not(shWindowResizable in SetupHeader.Options) then
  ...
  AppendMenu(SystemMenu, MF_STRING, 9999,
    PChar(SetupMessages[msgAboutSetupMenuItem]));
...
begin
  ...
  begin
    ...
    if SetupHeader.BackColor = SetupHeader.BackColor2 then
    begin
    ...
    begin
      ...
      begin
        ...
        if not(shBackColorHorizontal in SetupHeader.Options) then
          FillRect(Rect(0, MulDiv(CS.Y, Z, 255), CS.X,
            MulDiv(CS.Y, Z + 1, 255)))
        else
          FillRect(Rect(MulDiv(CS.X, Z, 255), 0, MulDiv(CS.X, Z + 1,
            255), CS.Y));
      end;
    end;
    ...
    SetFontNameSize(Font, LangOptions.TitleFontName, LangOptions.TitleFontSize,
      'Arial', 29);
...
begin
  ...
  S := S + ('Copyright (C) 1997-2020 Jordan Russell' + SNewLine +
    ...
    'All rights reserved.' + SNewLine2 + 'Inno Setup home page:' + SNewLine +
...
{$ENDIF}
  LoggedMsgBox(S, SetupMessages[msgAboutSetupTitle], mbInformation, MB_OK,
    False, 0);
end;
...
begin
  ...
  LoggedAppMessageBox(PChar(S), PChar(Application.Title), MB_OK or MB_ICONSTOP,
    True, IDOK);
end;
...
procedure TMainForm.SetStep(const AStep: TSetupStep;
...
begin
  ...
  WizardForm.FlipSizeAndCenterIfNeeded(shWindowVisible in SetupHeader.Options,
    MainForm, True);
...
begin
  ...
  if roRun32Bit in RunEntry.Options then
    Result := False;
  if roRun64Bit in RunEntry.Options then
  begin
    if not IsWin64 then
      InternalError
  ...
  try
    ...
    begin
      ...
      if not(roSkipIfDoesntExist in RunEntry.Options) or
        NewFileExistsRedir(DisableFsRedir, ExpandedFilename) then
      begin
        if not InstExecEx(RunAsOriginalUser, DisableFsRedir, ExpandedFilename,
          ExpandedParameters, ExpandConst(RunEntry.WorkingDir), Wait,
          RunEntry.ShowCmd, ProcessMessagesProc, ErrorCode) then
          raise Exception.Create(FmtSetupMessage1(msgErrorExecutingProgram,
            ExpandedFilename) + SNewLine2 +
            FmtSetupMessage(msgErrorFunctionFailedWithMessage,
            ['CreateProcess', IntToStr(ErrorCode),
            Win32ErrorString(ErrorCode)]));
    ...
    begin
      if not(roSkipIfDoesntExist in RunEntry.Options) or
        FileOrDirExists(ExpandedFilename) then
      begin
        if not InstShellExecEx(RunAsOriginalUser, ExpandConst(RunEntry.Verb),
          ExpandedFilename, ExpandedParameters,
          ExpandConst(RunEntry.WorkingDir), Wait, RunEntry.ShowCmd,
          ProcessMessagesProc, ErrorCode) then
          raise Exception.Create(FmtSetupMessage1(msgErrorExecutingProgram,
            ExpandedFilename) + SNewLine2 +
            FmtSetupMessage(msgErrorFunctionFailedWithMessage,
            ['ShellExecuteEx', IntToStr(ErrorCode),
            Win32ErrorString(ErrorCode)]));
      end
      else
...
function TMainForm.Install: Boolean;
  ...
  begin
    if Entries[seRun].Count <> 0 then
    begin
      CheckIfRestartNeeded := (shRestartIfNeededByRun in SetupHeader.Options)
        and not NeedsRestart;
      ...
      try
        ...
        begin
          ...
          if not(roPostInstall in RunEntry.Options) and
            ShouldProcessRunEntry(WizardComponents, WizardTasks, RunEntry) then
          begin
            ...
            if RunEntry.StatusMsg <> '' then
            begin
              try
                WizardForm.StatusLabel.Caption :=
                  ExpandConst(RunEntry.StatusMsg);
              except
                ...
                Application.HandleException(Self);
                WizardForm.StatusLabel.Caption :=
                  SetupMessages[msgStatusRunProgram];
              end;
            end
            else
              WizardForm.StatusLabel.Caption :=
            ...
            if roHideWizard in RunEntry.Options then
            begin
              if WizardForm.Visible and not HideWizard then
              begin
                HideWizard := True;
  ...
  begin
    ...
    begin
      WizardForm.StatusLabel.Caption := SetupMessages
...
begin
  ...
  try
    ...
    if RmDoRestart and (InitRestartApplications or
      ((shRestartApplications in SetupHeader.Options) and
      not InitNoRestartApplications)) then
      RestartApplications;
    ...
    if NeedsRestart and not InitNoRestart then
    begin
      with WizardForm do
      begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          if RunList.Checked[I] then
          begin
            { Disable windows before processing the first entry }
            if WindowDisabler = nil then
              WindowDisabler := TWindowDisabler.Create;
            RunEntry := PSetupRunEntry
              (Entries[seRun][Integer(RunList.ItemObject[I])]);
...
begin
  try
    ...
    if not FromPreparingPage and not NeedsRestart then
    begin
          ...
          imSilent:
            begin
              ...
              begin
                ...
                S := ExpandSetupMessage(msgFinishedRestartMessage);
              RestartSystem := LoggedMsgBox(S, '', mbConfirmation, MB_YESNO,
                True, IDYES) = IDYES;
            end;
          imVerySilent:
            RestartSystem := True;
...
begin
  ...
  if Assigned(WizardForm) and WizardForm.HandleAllocated and
    IsWindowVisible(WizardForm.Handle) and IsWindowEnabled(WizardForm.Handle)
    and WizardForm.CancelButton.CanFocus then
  begin
    case CurStep of
      ssPreInstall:
        if ConfirmCancel((WizardForm.CurPageID <> wpPreparing) or
          (WizardForm.PrepareToInstallFailureMessage = '')) then
        begin
      ...
      ssInstall:
        if (shAllowCancelDuringInstall in SetupHeader.Options) and not InitNoCancel
        then
...
begin
  ...
  if Assigned(WizardForm) and WizardForm.HandleAllocated then
  begin
    ShouldShow := WizardForm.Showing and not HideWizard and
      not IsIconic(Application.Handle);
    if (GetWindowLong(WizardForm.Handle, GWL_STYLE) and WS_VISIBLE <> 0) <> ShouldShow
    then
    begin
      if ShouldShow then
...
begin
  ...
  if Message.Show and (GetActiveWindow = Handle) and Assigned(WizardForm) and
    WizardForm.HandleAllocated and IsWindowVisible(WizardForm.Handle) then
...
var
  ...
  IsWow64ProcessFunc: function(hProcess: THandle; var Wow64Process: BOOL)
    : BOOL; stdcall;
  IsWow64Process2Func: function(hProcess: THandle;
    var pProcessMachine, pNativeMachine: USHORT): BOOL; stdcall;
...
begin
  ...
  if Assigned(IsWow64Process2Func) and IsWow64Process2Func(GetCurrentProcess,
    ProcessMachine, NativeMachine) and
    (ProcessMachine <> IMAGE_FILE_MACHINE_UNKNOWN) then
  begin
    IsWin64 := True;
    case NativeMachine of
      IMAGE_FILE_MACHINE_I386:
        ProcessorArchitecture := paX86;
      IMAGE_FILE_MACHINE_IA64:
        ProcessorArchitecture := paIA64;
      IMAGE_FILE_MACHINE_AMD64:
        ProcessorArchitecture := paX64;
      IMAGE_FILE_MACHINE_ARM64:
        ProcessorArchitecture := paARM64;
    else
  ...
  if IsWin64 and not(AreFsRedirectionFunctionsAvailable and
...
procedure InitWindowsVersion;
  ...
  var
    ...
    Size, Typ, SP: DWORD;
  begin
    if RegOpenKeyExView(rvDefault, HKEY_LOCAL_MACHINE,
      'System\CurrentControlSet\Control\Windows', 0, KEY_QUERY_VALUE, K) = ERROR_SUCCESS
    then
    begin
      Size := SizeOf(SP);
      if (RegQueryValueEx(K, 'CSDVersion', nil, @Typ, @SP, @Size)
        = ERROR_SUCCESS) and (Typ = REG_DWORD) and (Size = SizeOf(SP)) then
  ...
  begin
    if RegOpenKeyExView(rvDefault, HKEY_LOCAL_MACHINE,
      'System\CurrentControlSet\Control\ProductOptions', 0, KEY_QUERY_VALUE, K)
      = ERROR_SUCCESS then
    begin
      { See MS KB article 152078 for details on this key }
      if RegQueryStringValue(K, 'ProductType', S) then
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          NTServicePackLevel := (Byte(OSVersionInfoEx.wServicePackMajor) shl 8)
            or Byte(OSVersionInfoEx.wServicePackMinor);
...
SHGetKnownFolderPathFunc :=
  GetProcAddress(SafeLoadLibrary(AddBackslash(GetSystemDir) + shell32,
